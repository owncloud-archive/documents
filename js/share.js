/* global Documents, escapeHTML */

(function ($, Documents) {
	"use strict";

	/**
	 * @typedef {Object} Documents.Share.Types.ShareInfo
	...
	var Share = {
		...
		showDropDown: function (itemType, path, appendTo, link, possiblePermissions, filename) {
			...
			if (possiblePermissions & OC.PERMISSION_SHARE) {
				...
				// not.
				// FIXME public uploading is not supported in Documents
				...
				$('#shareWith').autocomplete({
					...
					select: function (event, selected) {
						...
						if (shareType === Documents.Share.SHARE_TYPE_REMOTE) {
							permissions =
						...
						Documents.Share.share(
							...
							function (data) {
								...
								Documents.Share._addShareWith(data.id, shareType, shareWith,
									selected.item.label,
					...
					var insert = $("<a>");
					var text = item.label;
					if (item.value.shareType === Documents.Share.SHARE_TYPE_GROUP) {
						text = text + ' (' + t('core', 'group') + ')';
					} else if (item.value.shareType === Documents.Share.SHARE_TYPE_REMOTE) {
					...
					return $("<li>")
						.addClass(
							...
							Documents.Share.SHARE_TYPE_GROUP) ? 'group' : 'user')
						.append(insert)
						.appendTo(ul);
				};
				// FIXME Emailing links is not supported in Documents
			...
			$('#dropdown').slideDown(OC.menuSpeed, function () {
				Documents.Share.droppedDown = true;
			});
		...
		hideDropDown: function (callback) {
			...
			$('#dropdown').slideUp(OC.menuSpeed, function () {
				Documents.Share.droppedDown = false;
				$('#dropdown').remove();
		...
		showLink: function (id, token, password) {
			...
			var link = parent.location.protocol + '//' + location.host +
				OC.generateUrl('/apps/' + Documents.appName + '/s/') + token;
		 ...
		 * @param path
		 *
		 * @returns {Documents.Share.Types.ShareInfo|Boolean}
		 * @private
		 */
		_loadShares: function (path) {
			...
			if (data === false) {
				OC.dialogs.alert(t('Documents', 'Error while retrieving shares'),
					t('core', 'Error'));
			}
...
$(document).ready(function () {
			...
			if (OC.Share.droppedDown) {
				if (path != $('#dropdown').data('path')) {
					OC.Share.hideDropDown(function () {
						OC.Share.showDropDown(itemType, path, appendTo, link,
							possiblePermissions);
					});
				} else {
				...
				OC.Share.showDropDown(itemType, path, appendTo, link, possiblePermissions);
			}
		}
	});
	...
	$(this).click(function (event) {
		...
		if (OC.Share.droppedDown && isMatched &&
	...
	$(document).on('click', '#dropdown .unshare', function () {
		...
		OC.Share.unshare(shareId, function () {
			$li.remove();
	...
	$(document).on('change', '#dropdown .permissions', function () {
		...
		OC.Share.setPermissions($li.attr('data-id'), permissions);
	});
			...
			if (oc_appconfig.core.enforcePasswordForPublicLink === false) {
				...
				OC.Share.share(
					...
					function (data) {
						...
						OC.Share.showLink(data.id, data.token, null);
					});
			} else {
		...
		if (linkPassText.val() != '' && (event.type == 'focusout' || event.keyCode == 13)) {
			...
			$.ajax({
				...
				if (oc_appconfig.core.enforcePasswordForPublicLink) {
					OC.Share.showLink(data.id, data.token, "password set");
				}
			}).fail(function (xhr) {
				var result = xhr.responseJSON;
				$loading.addClass('hidden');
